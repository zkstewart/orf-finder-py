...
while True:
        try:
                ...
                print('Fasta file failed to load. If you misspelt the name, try again.')
                continue
print('')
...
print('Starting the core processing of this script now. Regular backups will be saved and notification will be present in this text area.')

# Get the nucleotide (record) out of our list of nucleotides (records) and grab them ORFs!      # Note that I didn't write the code up to the len(pro) < minProLen line which means it hasn't been commented in any significant degree. This was taken from the biopython user guide.
for record in records:
        for strand, nuc in [(+1, record.seq), (-1, record.seq.reverse_complement())]:
                for frame in range(3):
                        ...
                        for i in range(len(splitProtein)):                                                      # Note that I have done a 'for i in range...' loop rather than a 'for value in splitProtein' loop which would have been simpler for a reason explained below on the 'elif i + 1 ==' line
                                ...
                                # Get the three versions of ORF from each region inbetween stop codons
                                if 'M' in str(acceptedPro):                             # Obtains a traditional methionine initiated ORF starting from the first methionine if there is one in the sequence
                                        mPro = startCodon.search(str(acceptedPro)).groups()[0]          
        ...
        if len(tempMList + tempAltList + tempNoneList) >= 1:
                # Append '-' entries to lists which have less entries than we want to pull to allow for 'for' loops to run without exceptions
                ...
                # Run a final size comparison to choose the best ORF
                for i in range(0, hitsToPull):
                        if len(tempSortedNoneList[-1]) > len(tempSortedAltList[-1]) + noCodonStringency and len(tempSortedNoneList[-1]) > len(tempSortedMList[-1]) + noCodonStringency:         # Again, we add the stringency values to help with determining priority of ORF ordering. Since this script will often be returning either 1, 3, or 5 potential ORFs, it is important that we order these in the most logical way
                                ...
                                tempSortedMList.pop()
                # Format and produce the output of this really cool piece of script
                for i in range(0, hitsToPull):                          
        ...
        if ongoingCount%10000 == 0 and os.path.isfile(os.getcwd() + '\\' + outputFileName + '.txt') == False:
            ...
            output = open(outputFileName + '.txt', 'a')
            output.write(outputText)
            output.close()
            print('Backup made after ' + str(ongoingCount) + ' sequences scanned for ORFs.')
            outputText = ''

# Dump the last few results to .txt after the script has finished, or create the output if there were less than 10,000 sequences
